...
const UserImageManager = ({ userImages, onItemClick, onImageUploaded, isUploading }) => {
    const fileInputRef = useRef(null);
        ...
        <Box>
            ...
            />
                <Box sx={{
                    display: 'grid',
                    gridTemplateColumns: 'repeat(3, 1fr)', // <<< TẠO RA 3 CỘT BẰNG NHAU
                    gap: 1.5, // Tương đương spacing={1.5}
                    maxHeight: 'calc(100vh - 220px)',
                    overflowY: 'auto'
                }}>
                    {userImages.length === 0 && !isUploading && (
                    <Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center', mt: 4, fontStyle: 'italic', width: '100%' }}>
                ...
                {userImages.map((img) => (
                    <DraggableSidebarItem data={{ id: img.id, url: img.url, type: 'image' }}>
                        <Card
                            ...
                            sx={{
                                // Thay đổi cursor thành 'pointer' để báo hiệu có thể click
                                cursor: 'pointer', 
                                transition: 'transform 0.2s, box-shadow 0.2s',
                                '&:hover': { transform: 'scale(1.04)', boxShadow: 3 },
                                aspectRatio: '1 / 1',
                        ...
                        >
                            <CardMedia component="img" image={img.url} alt={img.name}
                                sx={{
                                    ...
                                    // Giữ nguyên để không ảnh hưởng tới kéo thả
                                    pointerEvents: 'none' 
                                }}
                                onError={(e) => {
                                    const target = e.target; target.onerror = null;
...
    }, [blocksOrder, invitationType]);
    ...
    const handleUserImageFileUpload = useCallback(async (files) => {
        ...
        for (let i = 0; i < files.length; i++) {
            // Gửi file với key là 'assets' để khớp với middleware `upload.any()`
            // và logic xử lý của controller.
            formData.append(`file_${i}`, files[i]);
        }

        setIsUploading(true);
        try {
            // SỬA LỖI: Thay đổi endpoint sang API của admin dành cho việc tải lên hàng loạt
            const response = await api.post('/admin/assets/upload-batch', formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });

            // Backend trả về một map các URL, chúng ta cần chuyển nó thành mảng
            const urlMap = response.data.urls;
            const newImages = Object.values(urlMap).map(url => ({
                id: uuidv4(),
                name: url.split('/').pop(), // Lấy tên file từ URL
                url: url
            }));
    ...
    const renderSecondarySidebar = () => {
        ...
        switch (activeTool) {
            ...
            case 'user-images':
                return <Box sx={{ p: 2 }}><UserImageManager userImages={userUploadedImages} onItemClick={handleSidebarItemClick} onImageUploaded={handleUserImageFileUpload} isUploading={isUploading} /></Box>;
            case 'icons':
